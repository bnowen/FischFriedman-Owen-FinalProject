...
#Option 2 - PLotly? Seems like our best shot. But it seems to have a problem with bisalp?

```{r}
library(plotly)
fig <- plot_ly(byregion, x = ~hits, y = ~location, size = ~hits, color = ~keyword, frame= ~date, type = 'scatter', mode = "markers")
fig
fig <- fig %>% layout(
    title = "Search Results by Keyword",
...
fig <- fig %>%
  animation_opts(
    1000, easing = "cubic-in", redraw = FALSE
  )
fig
```
```{R}
