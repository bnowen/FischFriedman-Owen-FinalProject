...
- Can also rename dates to make them cleaner if preferred. 
...
library(plotly)
fig <- plot_ly(byregion, x = ~hits, y = ~location, size = ~hits, color = ~keyword, frame= ~date, type = 'scatter', mode = "markers", fill = ~'')

fig <- fig %>% layout(
    title = "Search Results by Top 5 Keywords",
    xaxis = list(title = "Hits"),
...
fig <- fig %>%
  animation_opts( #higher value makes animation slower
    5000, easing = "cubic-in", redraw = FALSE 
  )
fig
```
